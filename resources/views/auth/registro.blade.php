...
<div class="flex justify-center items-center min-h-[80vh] px-2 font-roboto">
    <div class="p-6 rounded-xl shadow-2xl shadow-[#00000048] w-full max-w-sm bg-[#E5CA17]">
        <h1 class="text-2xl font-bold text-center text-slate-800 mb-11 mt-7">Registrarse</h1>
        <form method="POST" action="{{ route('registrarse.post') }}" class="space-y-4">
            @csrf

            <div>
                <label for="name" class="block text-lg font-semibold text-slate-700">Nombre Completo</label>
            ...
            <div>
                <label for="email" class="block text-lg font-semibold text-slate-700">Correo Electrónico</label>
                <input type="email" name="email" id="email" value="{{ old('email') }}" required
                    class="w-full mt-1 p-2.5 rounded-lg border border-gray-300 focus:ring-yellow-400 focus:border-yellow-400 transition duration-200" />
            </div>

            <div>
                <label for="password" class="block text-lg font-semibold text-slate-700">Contraseña</label>
                <input type="password" name="password" id="password" required
                    class="w-full mt-1 p-2.5 rounded-lg border border-gray-300 focus:ring-yellow-400 focus:border-yellow-400 transition duration-200" />
            </div>

            <div>
                <label for="password_confirmation" class="block text-lg font-semibold text-slate-700">Confirmar Contraseña</label>
